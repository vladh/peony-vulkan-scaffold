namespace vulkan::geometry_stage {
  ...
  static void render(VkState *vk_state, VkExtent2D extent, u32 idx_image) {
    ...
    // Record command buffer
    {
      ...
      // Render
      rendering::render_drawable_component(&vk_state->sign, command_buffer);
  ...
  static void init_swapchain(VkState *vk_state, VkExtent2D extent) {
    ...
    // Descriptors
    {
      ...
      auto const pool_info = vkutils::descriptor_pool_create_info(N_PARALLEL_FRAMES, geometry_stage::N_DESCRIPTORS,
        geometry_stage::DESCRIPTOR_POOL_SIZES);
